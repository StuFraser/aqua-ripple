        ...
        >
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden">
                ...
                <div className="bg-aqua-dark px-5 py-4 shrink-0">
                    ...
                    {/* Overall quality pill + mode badge */}
                    <div className="mt-3 flex items-center gap-2 flex-wrap">
                        <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border text-sm font-bold ${qc.bg} ${qc.text} ${qc.border}`}>
                ...
                <div className="overflow-y-auto flex-1 px-5 py-4 flex flex-col gap-5">
                    ...
                    <p className="text-xs text-gray-400 leading-relaxed pb-1">
                        {data.fallback
                            ? "Generated by the deterministic rules engine from Sentinel-2 imagery — AI analysis was requested but Groq's rate limit was reached, so this result used the rule-based fallback instead."
                            : data.mode === "ai"
                                ? "AI-generated from Sentinel-2 imagery."
                                : data.mode === "rules"
                                    ? "Computed deterministically from Sentinel-2 spectral indices — no AI involved."
                                    : "Source not recorded for this older result."}
                        {" "}Not verified by environmental professionals — always check official advisories.
                    </p>
                </div>
            </div>
        </div>
